...
const FamilyScreen = ({ navigation }: any) => {
  const [labels, setLabels] = useState(["1000", "2000", "3000", "4000"]);
  ...
  const [tierNames, setTierNames] = useState(["0", "1", "2", "3"]);
  ...
  useEffect(() => {
    getAllTiers().then((tiers) => {
      ...
      newlabels[3] = tiers[0].tier4.toString();
      ...
      newDescriptions[3] = tiers[0].tier4description;
      const tierNames = [];
      tierNames[0] = tiers[0].tier1title;
      tierNames[1] = tiers[0].tier2title;
      tierNames[2] = tiers[0].tier3title;
      tierNames[3] = tiers[0].tier4title;
      setLabels(newlabels);
      ...
      if (familyBalance < tiers[0].tier2) {
        settierStep(0);
      } else if (
        familyBalance >= tiers[0].tier2 &&
        familyBalance < tiers[0].tier3
      ) {
        settierStep(1);
        console.log(tiers[0].tier2title);
      } else if (
        familyBalance >= tiers[0].tier3 &&
        familyBalance < tiers[0].tier4
      ) {
        settierStep(2);
      } else {
        settierStep(3);
    ...
    if (!dropdownExpanded) {
          ...
          <View style={styles.dropdownHeader}>
            ...
            <View style={styles.addContainer}>
              <View style={styles.dropdownList}>
                ...
                <Text style={[styles.nameText, { color: "#5F5F5F" }]}>
                  {parseNames()}
                </Text>
              </View>
      ...
      return (
        <View style={styles.dropdownContainer}>
          <Pressable
            onPress={() => setDropdownExpanded(!dropdownExpanded)}
            style={styles.familyCardContainer}
          >
            <View style={styles.dropdownHeader}>
              {iconStack()}
              <View style={styles.addContainer}>
                <View style={styles.dropdownList}>
                  <View style={styles.rowContainer}>
                    <Text
                      style={[globalStyles.body1Bold, { color: "#5F5F5F" }]}
                    >
                      {familyMembers.length} People
                    </Text>
                  </View>
          ...
          <View style={styles.memberRow}>
            ...
            <View style={styles.memberInfo}>
              <Text style={styles.memberName}>
                {familyMembers[0].full_name.split(" ")[0]}
              </Text>
              ...
              <>
                ...
                <View style={styles.memberRow}>
                  ...
                  <View style={styles.memberInfo}>
                    <Text style={styles.memberName}>
                      {member.role == "Child"
                        ? member.full_name.split(" ")[0] + ", not in balance"
                        : member.full_name.split(" ")[0]}
                    </Text>
                    <Text style={styles.memberPoints}>
                      {member.suspended ? "N/A" : member.points}
                    </Text>
                  </View>
                </View>
              </>
  ...
  const renderTierTitle = () => {
    if (tierStep == 0) {
      return "Tier 1: The Basic Family";
    } else if (tierStep == 1) {
      return "Tier 2: The Good Family";
    } else if (tierStep == 2) {
      return "Tier 3: The Better Family";
    } else {
      return "Tier 4: The Best Family";
    }
  };
        ...
        <View style={styles.buttomBar}>
          <View style={styles.tierContainer}>
            <View style={styles.tierBox}>
              <Text style={[styles.tierText, { color: "#253C85" }]}>
                {renderTierTitle()}
              </Text>
            </View>
            <Pressable
              onPress={() => setToggleExpanded(!toggleExpanded)}
              style={({ pressed }) => [
                {
            ...
            >
              <Text
                style={[
                  globalStyles.body2Bold,
                  { color: "#A9A9A9", marginTop: 2 },
                ]}
              >
                What Can I Get?
              </Text>
      ...
      return (
        <View style={styles.tierList}>
          ...
          <View style={styles.tierTitleContainer}>
            ...
            <Text
              style={tierStep == 0 ? styles.tierTitle : styles.tierTitleGray}
            >
              {tierNames[0]}
            </Text>
          </View>
          ...
          <View style={styles.tierTitleContainer}>
            ...
            <Text
              style={tierStep == 1 ? styles.tierTitle : styles.tierTitleGray}
            >
              {tierNames[1]}
            </Text>
          </View>
          ...
          <View style={styles.tierTitleContainer}>
            ...
            <Text
              style={tierStep == 2 ? styles.tierTitle : styles.tierTitleGray}
            >
              {tierNames[2]}
            </Text>
          </View>
          ...
          <Text style={styles.tierDescription}>{tierDescriptions[3]}</Text>
